import { read, type Pointer } from 'bun:ffi';
import type { BaseSDL } from '../../../..';
import type { GamepadDeviceEventType, RawGamepadDeviceEvent } from '../types';

export class GamepadDeviceEvent implements RawGamepadDeviceEvent {
  public static readonly BYTE_SIZE = 24;

  public type: GamepadDeviceEventType;
  public reserved: number;
  public timestamp: bigint;
  public which: number;
  public free: (() => void) | null;
  public address: Pointer | null;

  public constructor(options: RawGamepadDeviceEvent) {
    this.type = options.type;
    this.reserved = options.reserved;
    this.timestamp = options.timestamp;
    this.which = options.which;
    this.free = options.free;
    this.address = options.address;
  }

  public toMemory() {
    const buffer = GamepadDeviceEvent.allocMemory();
    const view = new DataView(buffer.buffer);

    view.setUint32(0, this.type, true);
    view.setUint32(4, this.reserved, true);
    view.setBigUint64(8, this.timestamp, true);
    view.setUint32(16, this.which, true);

    return buffer;
  }

  public static allocMemory() {
    const buffer = new Uint8Array(this.BYTE_SIZE);

    return buffer;
  }

  public static fromPointer(pointer: Pointer, sdl: BaseSDL) {
    const result = {
      type: read.u32(pointer, 0),
      reserved: read.u32(pointer, 4),
      timestamp: read.u64(pointer, 8),
      which: read.u32(pointer, 16),
      free: () => {
        sdl.symbols.SDL_free(pointer);
      },
      address: pointer,
    } as RawGamepadDeviceEvent;

    return new GamepadDeviceEvent(result);
  }

  public static fromMemory(data: Uint8Array) {
    const view = new DataView(data.buffer, data.byteOffset, data.byteLength);

    const result = {
      type: view.getUint32(0, true),
      reserved: view.getUint32(4, true),
      timestamp: view.getBigUint64(8, true),
      which: view.getUint32(16, true),
      free: null,
      address: null,
    } as RawGamepadDeviceEvent;

    return new GamepadDeviceEvent(result);
  }
}
